...
function chooseHandler(chooseId) {
	...
	$.ajax({
		type: "POST",
		data: "stuId="+studentId+"&examPaperId="+examPaperId+"&subjectId="+subjectId+"&index="+index+"&chooseAswer="+chooseAswer,
		url: "choose",
		success: function(data) {
			if(data.trim()=="f") {
				/*alert("提交失败，未知异常");*/
				console.log("提交失败，未知异常");
			}
		},
		error: function(data) {
...
function subTime() {
	...
	if(lastmin<=0 && lastsec<=1) {
		...
		zeroModal.show({
			...
			onClosed : function() {
				//考试结束回到考试中心页面
				location.href = "willexams?classId="+$('#examEndRefresh_classId').text().trim()+"&studentId="+$('#exam-studentId').text().trim()+"";
			}
		});
		return false;
	}
